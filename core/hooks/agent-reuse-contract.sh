...
#       claude) must carry a reuse contract in their prompt, otherwise they
#       start cold and reinvent what the repo already has → block.
...
case "$SUBAGENT" in
  general-purpose|claude)
    if ! echo "$PROMPT" | grep -qiF "reuse-before-reinvent"; then
      case "$USER_LANG" in
        ru) echo "BLOCKED: промпт build-агента ('$SUBAGENT') без reuse-before-reinvent контракта. Открой промпт agent-преамбулом (playbook §Контракт промпта агента): zoom-out + reuse-before-reinvent (grep/Explore существующие паттерны → переиспользуй) + контракт возврата. Добавь и повтори запуск." >&2 ;;
        *) echo "BLOCKED: a build-agent prompt ('$SUBAGENT') without a reuse-before-reinvent contract. Open the prompt with an agent preamble (playbook § Agent prompt contract): zoom-out + reuse-before-reinvent (grep/Explore existing patterns → reuse) + return contract. Add it and retry." >&2 ;;
      esac
      exit 2
    fi
    ;;
