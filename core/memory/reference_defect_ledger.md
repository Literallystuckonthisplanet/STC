...
| 2026-07-11 | **Enforcement hook matched a bare word, not real usage.** H16 keyed an integration off ANY occurrence of a generic-English service name (openai/stripe/aws/sheets/…), so the name in a comment, a string, a regex, or a **secret-pattern definition** (`"OpenAI-style key"` + an `sk-` regex in the leak-guard) produced a false docs-first block. | caught editing `deploy/checks.py` while adding token-format regexes to the leak-guard — H16 blocked on the word "openai" (worked around with `// ds-exception:` at the time, then fixed). | 🤖 narrow the trigger: generic-English service names now require a **USAGE** signal (import / API host / `*_api_key` / SDK client / network call); niche/regional names (cdek/modulbank/vk/…) still match on a bare mention. Same class as the earlier H15/H16 language-keyword false-positives (`import` in `python -c`, an `@anthropic.com` e-mail). Functional test covers pass/block. | ✅ closed |
