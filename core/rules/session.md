...
when you name a project). Need an overview of the infra itself (what codes/
hooks/skills exist) → read `core/memory/SNAPSHOT.md` (one line per code,
auto-regenerated by `deploy.py apply`) instead of scanning the `core/` tree.
