...
def build_snapshot(all_funcs, artifacts, retired, findings):
    ...
    lines = [
        # `name:` is what makes [[snapshot]] resolve — H08 builds its registry
        # of link targets from the name: frontmatter, not from filenames, so a
        # snapshot without it reads as a broken link in MEMORY.md.
        "---",
        "name: snapshot",
        "description: One-line-per-code catalog of the whole STC infra — read this"
        " instead of scanning core/ when asked what exists or where something lives.",
        "metadata:",
        "  type: reference",
        "---",
        "",
